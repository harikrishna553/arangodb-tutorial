...
public class DocumentReplaceDemo {
	...
	public static void main(String args[]) {
		...
		updatedDocument.addAttribute("name", "Krishna Gurram");
		updatedDocument.addAttribute("age", 33);

		DocumentUpdateEntity<BaseDocument> updatedEntity = collection.replaceDocument(key, updatedDocument);
		key = updatedEntity.getKey();
		persistedDoc = collection.getDocument(key, BaseDocument.class);
		System.out.println("\nReplaced entity : " + persistedDoc);
